...
function UserAvatarBtn() {
    ...
    const profileInfo = useSelector((state: RootState) => state.uiSclice.showProfileInfo);
    ...
    return (
        <div className="relative inline-flex" >
            ...
            <div
                className={`origin-top-right z-10 absolute top-12 right-0 min-w-44 bg-white border border-slate-200 py-1.5 rounded shadow-lg overflow-hidden mt-1 ${profileInfo ? " duration-300 " : " duration-300 -translate-y-[30rem] "}`} >
                <div
                    className=" flex justify-between pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200">
                    <div>
                        {client && (
                            <Link href={`/profile/${client._id}`} className="font-medium text-slate-800"> {client && `${client && client.firstName} ${client && client.lastName}`}
                            </Link>
                        )}
                    ...
                    <div>
                        <HiOutlineXCircle className="cursor-pointer" color="red" size={24}
                            onClick={() => disPatch(setShowProfileInfo())} />
                    </div>
                </div>
                <ul>
