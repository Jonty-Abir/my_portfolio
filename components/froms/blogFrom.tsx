...
function BlogFrom({ client, accessToken, refreshToken }: IpageProops) {
    ...
    const upload = async (event: ChangeEvent<HTMLInputElement>, setState: any) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (value) => {
            ...
            if (authorBase64 && posterBase64) {
                ...
                try {
                    const response = await createPost(payload, { accessToken, refreshToken });
    ...
    return (
        <form onSubmit={formik.handleSubmit} className=" text-gray-800 font-semibold">
            <div className="">
                {/* Panel body */}
                <div className="p-6">
                    <h2 className="text-2xl text-indigo-500 font-bold mb-5 flex items-center gap-x-2 ">Add Your Blog <FaBlog /></h2>

                    {/* Blog */}
                    {/* Tittle and Subtitle  */}
                    <section className='text-gray-800'>
                        <div className="text-sm text-gray-500 ">Let{"'"}s share some experience about your traveling, learning, weekend etc through your blogs.</div>
                ...
                {/* Panel footer */}
                <div className={`flex ${success || error ? "justify-between" : "flex-col"} px-6 py-5 border-t border-slate-200`}>
                    ...
                    <div className="flex self-end">
                        ...
                        <button disabled={loading ? true : false} type="submit" className="btn bg-indigo-600 hover:bg-indigo-700 text-white ml-3">{loading ? <LoadingBtn /> : <MdSend size={26} />}</button>
