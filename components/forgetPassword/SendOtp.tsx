...
function SendOtp({ setCount }: IComponentProps) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (value) => {
            try {
                ...
                toast.custom((t) => <NoficationToast t={t} msg={response?.msg} success={true} />)
                handleIncrement();
            } catch (error: any) {
                setLoading(false);
                setError(error?.message);
                toast.custom((t) => <NoficationToast t={t} msg={`Can't send OTP! try again..`} success={false} />);
            }
        },
    });
    ...
    return (
        <div className="max-w-sm mx-auto px-4 py-8 ">
            <h1 className="text-3xl text-slate-200 font-bold mb-6">Reset your Password ✨</h1>
            {/* Form */}
            <form onSubmit={formik.handleSubmit}>
                ...
                <div className="flex justify-between mt-6">
                    ...
                    <button type="submit" className="btn bg-indigo-600 hover:bg-indigo-600 text-white whitespace-nowrap">{loading ? "Sending..." : "Send Mail"}</button>
                </div>
            </form>
