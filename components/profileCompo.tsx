import { setACtiveAccountFrom, setACtiveBlogFrom } from '@/redux/sclice/clientSclice';
...
const ProfileCompo = () => {
    ...
    const accessToken = useSelector((state: RootState) => state.authSclice.accessToken);
    const refreshToken = useSelector((state: RootState) => state.authSclice.refreshToken);
        ...
        <>
            <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto bg-slate-300">
                ...
                <div className="mb-8">
                    ...
                    {/* Title */}
                    <h1 className="text-2xl md:text-3xl text-slate-800 font-bold">Account Settings <FcSettings className=' inline-block' /></h1>
                ...
                <div className="bg-white shadow-lg rounded-sm mb-8">
                    <div className="flex flex-col md:-mr-px lg:flex-row" >
                        ...
                        <div className="flex flex-nowrap overflow-x-scroll no-scrollbar md:block md:overflow-auto px-3 py-6 border-b md:border-b-0 md:border-r border-slate-200 min-w-60 md:space-y-3">
                            ...
                            <div>
                                ...
                                <ul className="flex lg:flex-col flex-nowrap gap-x-2 items-center space-y-2 md:block mr-3 md:mr-0">
                                    <li className={`mr-0.5 md:mr-0 md:mb-0.5 ${activeFrom.account && "liBorder"}`} onClick={() => disPatch(setACtiveAccountFrom(true))}>
                                        <a className="flex items-center gap-x-2 p-2 rounded whitespace-nowrap bg-indigo-50">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-user-shield stroke-indigo-500" width="32" height="32" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#597e8d" fill="none" strokeLinecap="round" strokeLinejoin="round">
                                    ...
                                    <li className={`mr-0.5 md:mr-0 md:mb-0.5 ${activeFrom.blog && "liBorder"}`} onClick={() => disPatch(setACtiveBlogFrom(true))}>
                                        <a className="flex items-center gap-2 p-2 rounded text-indigo-500 whitespace-nowrap bg-indigo-50">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-apps-filled " width="32" height="32" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#597e8d" fill="none" strokeLinecap="round" strokeLinejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                            ...
                                            <span className="text-sm font-medium text-indigo-500">Add Blog</span>
                                        </a>
                                    </li>
                                </ul>
                            </div>
                                ...
                                <ul className="flex flex-nowrap md:block mr-3 md:mr-0">
                                    <li className="mr-0.5 md:mr-0 md:mb-0.5">
                                        <a className="flex items-center px-2.5 py-2 rounded whitespace-nowrap" >
                                            ...
                                            <span className="text-sm font-medium text-slate-600 hover:text-slate-700">Give Feedback</span>
                                        </a>
                                    </li>
                                </ul>
                            </div>
                        ...
                        {/* Account */}
                        {activeFrom.account && (<ClientUpdateFrom client={client} accessToken={accessToken} refreshToken={refreshToken} />)}
                        {/* Blog */}
                        {activeFrom.blog && (<BlogFrom client={client} accessToken={accessToken} refreshToken={refreshToken} />)}
                    </div>
                </div>
