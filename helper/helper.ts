...
export async function register(payload: formikObj) {
    try {
        const { data, status } = await instance.post("/clients/reg", { ...payload });
...
export async function verifyAccesToken(accessToken: string, refreshToken: string) {
    try {
        const decoded: any = jwtDecode(accessToken);// just for decoded normal string

        const { data, status } = await instance.get(`/token/verify-token-client/${decoded._id}`, { headers: { Authorization: `Bearer ${accessToken} ${refreshToken}` } });
...
export async function sendOtp(payload: ISendOtpPayload) {
    try {
        const { data, status } = await instance.post("/reset-password/send-otp", { ...payload });
...
export async function verifyOtp(payload: IVerifyOtpPayload) {
    try {
        const { data, status } = await instance.post("/reset-password/verify-otp", { ...payload });
...
export async function updateClient(id: string, payload: IupdateClientPayload, tokens: Itokens) {
    try {

        const { data, status } = await instance.patch(`/auth/client/${id}`, { ...payload }, { headers: { Authorization: `Bearer ${tokens.accessToken} ${tokens.refreshToken}` } });
...
export async function findOne(id: string, token: string, refreshToken: string) {
    try {
        ...
        const { data, status } = await instance.get(`/auth/client/${id}`, { headers: { Authorization: `Bearer ${token} ${refreshToken}` } });
        if (status === 200 || status === 304) return data;
        throw new Error("Error was occure!");
    } catch (error: any) {
...
export async function findAllClient(accessToken: string, refreshToken: string) {
    try {
        ...
        const { data, status } = await instance.get(`/clients`, { headers: { Authorization: `Bearer ${accessToken} ${refreshToken}` } });
        if (status === 200 || status === 304) return data;
        throw new Error("Error was occure!");
    } catch (error: any) {
        console.log("hey react query..");
